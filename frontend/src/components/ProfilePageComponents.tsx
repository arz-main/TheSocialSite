...
export const TabsPage: React.FC<TabsPageProps> = ({
    ...
    const paginatedDrawings = userDrawings.slice(
        (drawingsPage - 1) * PAGE_SIZE,
        drawingsPage * PAGE_SIZE
    );
    const totalDrawingsPages = Math.ceil(userDrawings.length / PAGE_SIZE);

    const paginatedPosts = userPosts.slice(
        (postsPage - 1) * PAGE_SIZE,
        postsPage * PAGE_SIZE
    );
    const totalPostsPages = Math.ceil(userPosts.length / PAGE_SIZE);
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {badges.map((badge, index) => (
                            ...
                            >
                                <Card className={`p-6 transition-all ${badge.earned ? "hover:shadow-lg" : "opacity-50 grayscale"}`}>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    {userPosts.length === 0 ? (
                        ...
                        <>
                            ...
                            {userPosts.length > PAGE_SIZE && (
                                <div className="flex justify-center gap-2 mt-4">
                                    <Button onClick={() => setPostsPage((p) => Math.max(p - 1, 1))} disabled={postsPage === 1}>
                                        Prev
                                    </Button>
                                    <span className="px-2 py-1">{postsPage}</span>
                                    <Button onClick={() => setPostsPage((p) => Math.min(p + 1, totalPostsPages))} disabled={postsPage >= totalPostsPages}>
                                        Next
                                    </Button>
                                </div>
                            )}
